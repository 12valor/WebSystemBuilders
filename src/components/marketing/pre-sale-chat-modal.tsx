...
export function PreSaleChatModal({
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-3.5 sm:p-4 backdrop-blur-sm font-sans">
        <motion.div
          initial={{ opacity: 0, scale: 0.95, y: 10 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.95, y: 10 }}
          className="relative w-full max-w-lg max-h-[90dvh] overflow-y-auto overscroll-contain rounded-3xl border border-slate-200 bg-white p-5 sm:p-8 shadow-2xl"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-slate-100 pb-3.5 sm:pb-4">
            <div className="flex items-center gap-3">
              <div className="size-10 rounded-2xl bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-600 shrink-0">
              ...
              <div className="min-w-0">
                <h3 className="text-sm sm:text-base font-bold text-slate-900">Pre-Sale Owner Chat</h3>
                <p className="text-xs text-slate-500 leading-snug">Ask questions about systems, scope, or delivery before purchasing.</p>
              </div>
            </div>
            ...
            <button
              type="button"
              onClick={onClose}
              className="flex size-9 shrink-0 items-center justify-center rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
          ...
          <div className="mt-5 space-y-4 sm:mt-6">
            {loadingUser ? (
              ...
              <div className="rounded-2xl border border-blue-100 bg-blue-50/60 p-5 sm:p-6 text-center space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-center pt-2">
                  <Link
                    ...
                    onClick={onClose}
                    className="blue-button flex min-h-[44px] items-center justify-center bg-blue-600 px-5 py-2.5 text-xs font-bold text-white"
                  >
                    Sign In
                  </Link>
                  <Link
                    href="/auth/sign-up"
                    onClick={onClose}
                    className="flex min-h-[44px] items-center justify-center rounded-xl border border-slate-200 bg-white px-5 py-2.5 text-xs font-semibold text-slate-700 hover:bg-slate-50 transition"
                  >
                    Create Account
                  </Link>
                </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-slate-900 font-mono truncate max-w-[200px]">{user.email}</span>
                </div>

                {statusMessage && (
                  <div
                    className={`rounded-xl border p-3.5 sm:p-4 text-xs font-semibold ${
                      status === "success"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="blue-button flex min-h-[44px] w-full sm:w-auto items-center justify-center bg-blue-600 px-5 py-2.5 text-xs font-bold text-white disabled:opacity-50"
                  >
