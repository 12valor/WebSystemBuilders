...
describe("production launch environment", () => {
  ...
  it("rejects invalid keys, foreign senders, non-canonical origins, and missing secrets", () => {
    const issues = checkLaunchReadiness({ ...ready, RESEND_FROM_EMAIL: "delivery@example.com", SITE_URL: "http://localhost:3000", SUPABASE_SERVICE_ROLE_KEY: "" });
    expect(issues.map((issue) => issue.code)).toEqual(expect.arrayContaining(["resend_sender", "canonical_site_url", "supabase_service_role"]));
  });
});
