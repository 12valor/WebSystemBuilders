...
const ready = {
  ...
  PAYPAL_CLIENT_ID: "paypal_client_example_1234567890",
  PAYPAL_CLIENT_SECRET: "paypal_secret_example_1234567890",
  PAYPAL_ENVIRONMENT: "live",
  PAYPAL_WEBHOOK_ID: "paypal_webhook_1234567890",
...
describe("production launch environment", () => {
  ...
  it("rejects invalid keys, foreign senders, non-canonical origins, and missing secrets", () => {
    const issues = checkLaunchReadiness({ ...ready, RESEND_FROM_EMAIL: "delivery@example.com", SITE_URL: "http://localhost:3000", SUPABASE_SERVICE_ROLE_KEY: "", PAYPAL_CLIENT_SECRET: "", PAYPAL_ENVIRONMENT: "sandbox" });
    expect(issues.map((issue) => issue.code)).toEqual(expect.arrayContaining(["resend_sender", "canonical_site_url", "supabase_service_role", "paypal_client_secret", "paypal_environment"]));
  });
});
