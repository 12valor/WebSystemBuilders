...
export default async function CheckoutPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><SiteHeader /><main id="main-content" className="py-12 sm:py-16 lg:py-24"><div className="mx-auto grid w-[min(calc(100%-40px),1080px)] gap-8 md:w-[min(calc(100%-64px),1080px)] lg:grid-cols-[minmax(0,.9fr)_minmax(360px,.7fr)] lg:gap-14">
    <section><p className="text-xs font-semibold uppercase tracking-[0.1em] text-muted">Secure checkout</p><h1 className="mt-4 text-4xl font-semibold tracking-[-0.05em] sm:text-5xl">Review before payment.</h1><p className="mt-5 max-w-xl leading-7 text-secondary">Your payment is processed securely via Lemon Squeezy Merchant of Record.</p>
    ...
    <section className="h-fit rounded-2xl border border-white/10 bg-surface p-6 sm:p-7"><h2 className="text-xl font-semibold tracking-[-0.03em]">Buyer information</h2><p className="mt-2 text-sm leading-6 text-secondary">Use an email address you can access. Delivery will be tied to it after verified payment.</p>{configured ? <div className="mt-6"><CheckoutForm systemSlug={system.slug} /></div> : <div role="status" className="mt-6 rounded-xl border border-amber-400/30 bg-amber-400/[0.06] p-4 text-sm leading-6 text-amber-100">Lemon Squeezy credentials are not connected yet. Checkout is visible for review, but no order or charge can be created.</div>}</section>
  </div></main><SiteFooter /></>;
}
