...
export function getIntegrationHealth(): IntegrationHealthItem[] {
  ...
  const paymongo = hasMinimum(process.env.PAYMONGO_SECRET_KEY, 20) && hasMinimum(process.env.PAYMONGO_WEBHOOK_SECRET, 16);
  ...
  return [
    ...
    item("paymongo", "PayMongo payments", paymongo, "Requires both the server secret and webhook-verification secret."),
