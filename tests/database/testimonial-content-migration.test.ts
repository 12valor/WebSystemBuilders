...
describe("Phase 5 testimonial migration", () => {
  ...
  it("audits safe metadata without testimonial text, evidence, seed data, or deletion", () => { const audit = migration.slice(migration.indexOf("private.audit_testimonial_change")); expect(audit).toContain("'testimonial.published'"); expect(audit).not.toContain("source_reference"); expect(audit).not.toContain("new.quote"); expect(migration).not.toMatch(/delete from public\.testimonials/i); const schemaSection = migration.slice(0, migration.indexOf("create or replace function public.create_testimonial_draft")); expect(schemaSection).not.toMatch(/insert into public\.testimonials/i); });
});
